...
class SQLiteIdempotencyStore:
    ...
    def __init__(self, connection: sqlite3.Connection) -> None:
        ...
        self.connection.execute(
            "CREATE TABLE IF NOT EXISTS delivery_keys (key TEXT PRIMARY KEY)"
        )
        self.connection.commit()

    def seen(self, key: str) -> bool:
        return self.connection.execute("SELECT 1 FROM delivery_keys WHERE key=?", (key,)).fetchone() is not None
    ...
    def claim(self, key: str) -> bool:
        cursor = self.connection.execute("INSERT OR IGNORE INTO delivery_keys(key) VALUES(?)", (key,))
        self.connection.commit()
...
class SQLiteDeliveryHistory:
    ...
    def all(self) -> tuple[DeliveryRecord, ...]:
        rows = self.repository.connection.execute("SELECT * FROM delivery_history ORDER BY created_at DESC").fetchall()
        return tuple(DeliveryRecord(
            r["article_id"], r["destination"], bool(r["success"]), r["attempts"],
            r["external_id"], r["error"], __import__("datetime").datetime.fromisoformat(r["created_at"])
        ) for r in rows)
