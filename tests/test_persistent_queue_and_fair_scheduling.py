...
def test_fair_scheduling_and_priority_ordering(repo):
    ...
    for i in range(1, 16):
        ...
        repo.publication_queue.add_job(
            PublicationJob(f"a_{i}:pwa", f"a_{i}", "pwa", "pending", 10, "normal", "sourceA", created_at=now - timedelta(minutes=i))
        )
    ...
    for i in range(1, 3):
        ...
        repo.publication_queue.add_job(
            PublicationJob(f"b_{i}:pwa", f"b_{i}", "pwa", "pending", 10, "normal", "sourceB", created_at=now - timedelta(minutes=i))
        )
...
def test_retry_backoff_and_dead_letter(repo):
    ...
    processor = PublicationQueueProcessor(
        repo, [failing_publisher], base_backoff_seconds=10.0
    )
