...
def test_termux_installer_is_fail_fast_and_bootstraps_canonical_ai() -> None:
    ...
    assert "pkg install -y python git clang make pkg-config openssl openssl-tool libffi cmake patchelf ruff" in text
    ...
    assert 'python -m pip install "pytest>=8.2,<10"' in text
